...
| 2 | P14 | Deeper graph | In progress. **P14-S1 done** (`imports` graph edges; PR #40). **P14-S2 done** (`passes` graph edges: A renders B passing prop P, grounded in jsx-attribute facts, GraphEdge widened with optional `propNames`, merge-before-dedupe, ambiguity guard; PR #41). Remaining: prop drilling analyzer (S3, needs passes edges), `exportKind` predicate (S4), `calls` edges (S5). | Substrate. Richer model that more/better analyses and proposals stand on. |
