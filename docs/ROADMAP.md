...
| 2 | P14 | Deeper graph | In progress. **P14-S1 done** (`imports` graph edges; PR #40). **P14-S2 done** (`passes` graph edges; PR #41). **P14-S3 done** (`react/prop-drilling` analyzer — first passes-edge consumer, depth-2 name-level pass-through, common-name guard, self-edge guard; adapter-react; PR #42). Remaining: `exportKind` predicate (S4), `calls` edges (S5, needs S1 import grounding). | Substrate. Richer model that more/better analyses and proposals stand on. |
