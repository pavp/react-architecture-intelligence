...
| 3 | P16 | Safe code proposals | **P16-S1 done** — `react/prop-drilling` preview-only proposal: `PreviewProposal` type + `ProposalBuilder` interface + dispatch seam in core; pure `buildPropDrillingProposalBuilder` builder in adapter-react; CLI composition root wires `proposalBuilders`; preview-only enforced type-level (79 test files / 767 tests). **P16-S2 done** — `find_proposals` read-only MCP discovery tool: `Session.findProposals(opts?)` + `FindProposalsResult` union type; actionability mirrors `proposeRefactor` exactly via private `isActionable(ruleId)`; `find_proposals` registered in MCP server; agreement-invariant test (79 test files / 780 tests). **mcp-agent-ux shipped** (ad-hoc UX improvement, not a roadmap slice) — additive agent-discoverability hints on `raw_graph_query` (adds `total` pre-slice count) and `query_architecture` (adds static recovery `hint`); 782 tests passing. Next: P16-S3 apply-path for `react/prop-drilling` proposals (requires apply pipeline extension). | Actionability — findings→fixes, the biggest user leap. Safe only on calibrated findings (P13) + deeper graph (P14). |
...
| P16-S2 | `find_proposals` read-only discovery tool | Implemented (branch `feat/p16-s2-find-proposals`). `FindProposalsResult` union type + `Session.findProposals(opts?)` in `packages/core/src/mcp/tools.ts`; private `isActionable(ruleId)` helper mirrors `proposeRefactor` predicate (SHARED_EXTRACTION_RULE_ID \|\| proposalBuilders match — single definition); subject projected generically from evidence (undefined for shared-extraction, component name for adapter-metric, never fabricated); suppression filter and no_analysis guard reuse existing patterns; sorted fingerprint asc / ruleId asc; `find_proposals` MCP tool registered in `buildMcpServer` with `ruleId?` (string, optional) + `includeSuppressed?` (boolean, default false); agreement-invariant test locks correctness: every findProposals-actionable fingerprint must be accepted by proposeRefactor and vice versa. framework-free guard green (no React names in core). 79 test files / 780 tests (+13 new). |
| P16-S1 | Prop-drilling preview-only proposal | Implemented (branch `feat/p16-prop-drilling-preview-proposal`). `PreviewProposal` type (status/kind/ruleId/subject/observations/consider/limits/writeMode) + `ProposalBuilder` interface + `ProposalBuilderInput` in `packages/core/src/codemod/proposal.ts`. `SessionOpts.proposalBuilders?: ProposalBuilder[]` + dispatch seam in `Session.proposeRefactor`: shared-extraction first (unchanged), then builder lookup by ruleId, no-builder → unsupported-rule refusal. Limits from analyzer explain hook verbatim via existing registry seam. `buildPropDrillingProposalBuilder()` pure builder in `packages/adapter-react/src/prop-drilling-proposal.ts`: 1:1 grounded from AdapterMetricEvidence roles/metrics/subject; fixed non-prescriptive consider[] template (Context/shared hook/prop consolidation + "RAI does not know which is correct"); limits passed through verbatim; drilledProps sorted; no patch/diff/write. CLI composition root (`packages/cli/src/adapters.ts`) collects proposal builders via `ReactAdapterModule.buildPropDrillingProposalBuilder`. Preview-only enforced type-level — `PreviewProposal` not assignable to patch input; `applyRefactor` hardcoded to shared-extraction. framework-free guard green. 79 test files / 767 tests. |
| (Ad-hoc) | agent-instructions-pass | Shipped (not a numbered roadmap phase). MCP tool description accuracy pass + README install/CLI/signals/calibration refresh. Pays P9-S3 deferred description copy-pass + README onboarding work. Two chained slices: S1 agent-facing (expand install template, rewrite 9 MCP descriptions, add phrase-level description tests) merged PR #57; S2 human-facing (install UX correction, CLI table, signals table, calibration section, find_proposals example) on PR #58 APPROVED/PASS. Specs synced: mcp-tools.md (9 modified + description-invariant + phrase-test requirements) + new distribution-install.md canonical. Test baseline: 79 files / 785 tests (unchanged). |
