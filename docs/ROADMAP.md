...
| P11 | React Pattern Analyzers + Pattern Drift | In progress. P11-S1 complete: `react/compound-component-api-drift`; P11-S2 complete: `react/container-presenter-role-drift`; P11-S3 complete: `react/controlled-uncontrolled-prop-surface-drift`; P11-S4 complete: framework-neutral `call-binding`, `call-argument`, and `jsx-attribute` facts; P11-S5 implemented: `react/context-provider-value-surface-drift`, the first analyzer consuming P11-S4 facts. Remaining slices use those facts for more repo-derived adapter-owned patterns: forms, data fetching, design-system usage, overlays, and API conventions. |
...
- forms, data fetching, design-system usage, overlays beyond compound primitives, and broad API conventions.
- P11-S5 delivered the first analyzer slice consuming P11-S4 expanded facts (`react/context-provider-value-surface-drift`); later slices follow the same adapter-owned pattern.
