...
| 2 | P14 | Deeper graph | In progress. **P14-S1 done** (`imports` graph edges materialized in core from `PatternImportFact`: relative-only fs-free resolver, escaping-root guard, dedup/self/cycle handling; PR #40). Remaining: `passes` edges (S2), prop drilling (S3), `exportKind` predicate (S4), `calls` edges (S5). | Substrate. Richer model that more/better analyses and proposals stand on. |
