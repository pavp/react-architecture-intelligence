...
> **KNOWN ISSUE (blocks usable release) — engine not bundled.** v0.2.0/0.2.1/0.2.2 all INSTALL and the launcher starts (archive-mode resolves), but `rai analyze`/`rai doctor` CRASH with `ERR_MODULE_NOT_FOUND` because the release archive ships only the `index.js` stub — the prepare script (`scripts/prepare-release-dry-run.sh`) is a dry-run that never bundled the full engine (13 CLI files + @rai/core + @rai/adapter-react + 3 native deps better-sqlite3/sqlite-vec/oxc-parser + schema.sql). The earlier bugs (engine-not-found, platform-mismatch) masked this. **Fix designed + prototyped** (works on darwin/arm64): see engram `sdd/p8y-engine-bundle/explore` (id 965) + `sdd/p8y-engine-bundle/prototype` (id 966) and `scripts/bundle-engine.mjs`. Implementation = S1 (esbuild bundle + co-located node_modules/schema.sql + goreleaser archive rewrite + smoke gate that EXECUTES the engine) + S2 (per-arch CI matrix macOS/Linux/Windows for native deps + restore launcher arch guard). NOT yet implemented — next session. Until then, `rai` is usable only from a dev checkout (`mode: dev`), not from a published release.

Latest published release:

- `v0.2.2` (current — installs + launcher starts, but engine analysis broken, see KNOWN ISSUE above): first cross-platform-installable launcher. Fixes v0.2.1 platform-mismatch — a single static `metadata.json` (build-host linux/amd64) was copied into all 6 archives (GoReleaser OSS can't template archive file contents) and the launcher rejected every non-matching platform. Fix: removed the self-defeating platform check from `validateMetadata` (archive bundles no per-arch native code — pure JS on system-node; a wrong-arch binary can't exec anyway), and inject real per-target identity via ldflags (`launcherVersion`/`gitCommit`/`buildDate`). PR #45, squash b55daf8. NOTE: when P8-S3 ships per-arch native assets, re-add an arch guard keyed off the binary's ldflags identity (see launcher.go comment). Deferred (S2): real `enginePackageVersion` (still ships `0.0.0` placeholder) + cross-platform smoke-gate coverage.
