...
| Product state | P0–P11 complete plus P9-S3; P11 shipped 9 React pattern analyzer slices (S1–S9), all merged to `main`; P11-S10 (API conventions) deferred as ungroundable on syntax-only facts; first installable release `v0.1.3` published through GitHub Release, Homebrew tap, and Scoop bucket; **`v0.2.6` is the current release** — the first genuinely FUNCTIONAL multi-platform release (engine bundled per-platform via p8y-engine-bundle; 4 archives darwin/arm64 + linux amd64/arm64 + windows/amd64 + checksums). v0.2.0–v0.2.5 are superseded (v0.2.0–v0.2.2 installed but engine analysis crashed; v0.2.3–v0.2.5 were failed/cancelled CI runs while bundling was hardened across platforms). P13-S1 (`rai calibrate` suggest-only) implemented. P13-S2 (evidence-correlated suggestions) implemented. P13-S3 (`rai calibrate --apply` guarded write) implemented. P13-S2.x (`maxFanOut` secondary calibrated knob) implemented and merged (PR #39). P14-S1 (`imports` graph edges) implemented and merged (PR #40). P14-S2 (`passes` graph edges) implemented and merged (PR #41). P14-S3 (`react/prop-drilling` analyzer) implemented and merged (PR #42). P14-S5 (`calls` graph edges) implemented and merged (PR #43). **P14 COMPLETE** (S4 exportKind killed as ungroundable). **P16-S1 (`react/prop-drilling` preview-only safe code proposal) implemented** — `PreviewProposal` type + `ProposalBuilder` interface + `Session.proposeRefactor` dispatch seam in `@rai/core`; `buildPropDrillingProposalBuilder` pure builder in `@rai/adapter-react`; CLI composition root wires `proposalBuilders`; preview-only enforced type-level (no patch, diff, apply, or write). 79 test files / 767 tests (post-P16-S1 baseline). **P16-S2 (`find_proposals` read-only MCP discovery tool) implemented** — `FindProposalsResult` union type + `Session.findProposals(opts?)` read-only aggregation over `lastPresented`; private `isActionable(ruleId)` helper mirrors `proposeRefactor` predicate exactly; `find_proposals` MCP tool registered in `buildMcpServer`; agreement-invariant test confirms `findProposals` actionable set === `proposeRefactor` accepted set. 79 test files / 780 tests. **P16-S1–S2 COMPLETE**. |
| Next phase | P16-S3 apply-path for `react/prop-drilling` proposals (future work). P12 CI/PR remains last. |
...
pnpm test       # 79 files / 782 tests (post-mcp-agent-ux additions; v0.2.0 baseline was 78 files / 719 tests)
pnpm typecheck
pnpm build
pnpm lint
git diff --check
gh release view v0.2.6 --repo pavp/react-architecture-intelligence
...
> **RESOLVED in v0.2.6 — engine now bundled per-platform (p8y-engine-bundle).** The prior KNOWN ISSUE (v0.2.0–v0.2.2 installed but `rai analyze`/`rai doctor` crashed with `ERR_MODULE_NOT_FOUND` because the archive shipped only the `index.js` stub) is fixed. p8y-engine-bundle shipped in two slices: S1 (PR #46 — esbuild single-file bundle + co-located `schema.sql` + pruned native `node_modules` via `pnpm deploy`, goreleaser archive rewrite, and a smoke gate that EXECUTES `rai doctor` + `rai analyze` against the unpacked archive) and S2 (PR #47 — per-arch CI native matrix + launcher arch guard from the binary's Go build target). Three follow-up fixes after the first real multi-platform CI runs surfaced platform bugs invisible on darwin/arm64: Windows esbuild spawn → esbuild JS API (PR #49), win32 oxc binding `-msvc` suffix (PR #50), and dropping the unsupported darwin/amd64 / deprecated macos-13 runner (PR #51). The execute-the-engine smoke gate runs pre-publish in `release.yml`, so the bug class that broke v0.2.0–v0.2.2 can no longer reach users.

Latest published release:

- `v0.2.6` (current — first FUNCTIONAL multi-platform release): engine bundled per-platform via p8y-engine-bundle (PRs #46–#51). 4 archives (darwin/arm64, linux amd64/arm64, windows/amd64) + checksums; each ships the esbuild bundle + co-located `schema.sql` + pruned native `node_modules` (better-sqlite3/bindings/sqlite-vec/oxc-parser). The pre-publish smoke gate executes `rai doctor` + `rai analyze` against the unpacked archive. darwin/amd64 (Intel Mac) and windows/arm64 are intentionally not built (unsupported targets / missing native packages). Release workflow run 27585362814 completed/success including the Gated release job. NOTE: Homebrew tap + Scoop bucket may still reference an older version — update them to 0.2.6 if not auto-bumped.
- `v0.2.2` (superseded — installs + launcher starts, but engine analysis broken): first cross-platform-installable launcher. Fixes v0.2.1 platform-mismatch — a single static `metadata.json` (build-host linux/amd64) was copied into all 6 archives (GoReleaser OSS can't template archive file contents) and the launcher rejected every non-matching platform. Fix: removed the self-defeating platform check from `validateMetadata` (archive bundles no per-arch native code — pure JS on system-node; a wrong-arch binary can't exec anyway), and inject real per-target identity via ldflags (`launcherVersion`/`gitCommit`/`buildDate`). PR #45, squash b55daf8. NOTE: when P8-S3 ships per-arch native assets, re-add an arch guard keyed off the binary's ldflags identity (see launcher.go comment). Deferred (S2): real `enginePackageVersion` (still ships `0.0.0` placeholder) + cross-platform smoke-gate coverage.
...
| mcp-agent-ux | Complete | Ad-hoc MCP UX improvement (not a numbered roadmap phase): additive agent-UX hints on `raw_graph_query` and `query_architecture` tools to close agent-discoverability frictions. `raw_graph_query` success shape adds `total: number` (full pre-slice row count, proves truncation magnitude) + updated description naming MATCH nodes as target-enumeration path. `query_architecture` unknown_target adds static `hint: string` pointing to raw_graph_query + MATCH nodes for target discovery. 3 files modified (tools.ts, tools.test.ts, server.ts), ~51 net lines. 79 test files / 782 tests (+2 new tests on total/hint, strict pre-slice assertion locks correctness). framework-free guard green. Purely additive; existing fields/branches untouched. PR #56 APPROVED via Judgment Day, verify PASS (0 CRITICAL). Spec delta merged into canonical `openspec/specs/mcp-tools.md` (2 modified + 3 added requirements). Change archived to `openspec/changes/archive/2026-06-19-mcp-agent-ux/`. |
| P16-S2 | Complete | `find_proposals` read-only MCP discovery tool: `FindProposalsResult` union type (`{ actionable: Array<{fingerprint,ruleId,subject?}>; count }` | `{ status: "no_analysis"; message }`) + `Session.findProposals(opts?)` in `packages/core/src/mcp/tools.ts`; private `isActionable(ruleId)` mirrors `proposeRefactor` predicate exactly (SHARED_EXTRACTION_RULE_ID || proposalBuilders match); subject projected generically from evidence (`(evidence as {subject?:{name?:string}}).subject?.name` — undefined for shared-extraction, component name for adapter-metric); suppression filter and no_analysis guard reuse existing patterns; sorted fingerprint asc / ruleId asc; `find_proposals` tool registered in `buildMcpServer` (`packages/core/src/mcp/server.ts`) with `ruleId?` + `includeSuppressed?` params; agreement-invariant test locks findProposals ↔ proposeRefactor equivalence; framework-free guard stays green. 79 test files / 780 tests (+13 new tests, +12 assertions on agreement invariant). PR #55 APPROVED via Judgment Day. Spec delta merged into canonical `openspec/specs/mcp-tools.md`. Change archived to `openspec/changes/archive/2026-06-19-p16-s2-safe-code-proposals/`. |
| agent-instructions-pass | Complete | Ad-hoc UX change (not a numbered roadmap phase): MCP tool description accuracy pass + README install/CLI/signals/calibration refresh. Pays P9-S3 deferred description copy-pass + README onboarding work. Two chained slices (auto-chain, separate PRs): S1 (agent-facing — expand install template, rewrite 9 MCP tool descriptions, add phrase-level description tests: ~150–200 lines, 5 files) merged PR #57 at e968f34; S2 (human-facing — install UX correction, CLI table completeness, signals table expansion, calibration section, find_proposals agent example: ~60–80 lines, README.md only) on feat/agent-instructions-s2-readme PR #58 APPROVED via Judgment Day, verify PASS (0 CRITICAL). Test baseline: 79 files / 785 tests (unchanged post-S1, S2 README-only). Specs synced: mcp-tools.md (9 modified + description-invariant + phrase-test requirements) + new distribution-install.md canonical. Changes archived to `openspec/changes/archive/2026-06-19-agent-instructions-pass/`. |
| P16-S1 | Complete | `react/prop-drilling` preview-only safe code proposal: `PreviewProposal` type (status/kind/ruleId/subject/observations/consider/limits/writeMode) + `ProposalBuilder` interface + `ProposalBuilderInput` in `packages/core/src/codemod/proposal.ts`; `SessionOpts.proposalBuilders?: ProposalBuilder[]` + dispatch seam in `Session.proposeRefactor` (shared-extraction first, then builder lookup by ruleId, no-builder → unsupported-rule); limits from analyzer explain hook verbatim; `buildPropDrillingProposalBuilder()` pure builder in `packages/adapter-react/src/prop-drilling-proposal.ts` (grounded 1:1 from AdapterMetricEvidence roles/metrics/subject; fixed non-prescriptive consider[] template; limits passed through verbatim; drilledProps sorted; no patch/diff/write); CLI composition root (`packages/cli/src/adapters.ts`) wires proposal builders via ReactAdapterModule interface; preview-only enforced type-level (`PreviewProposal` not assignable to patch input; `applyRefactor` hardcoded to shared-extraction). 79 test files / 767 tests. framework-free guard green (no React names in core). PR #54 APPROVED via Judgment Day. Spec merged into canonical `openspec/specs/mcp-tools.md`. Change archived to `openspec/changes/archive/2026-06-19-p16-safe-code-proposals/`. |
