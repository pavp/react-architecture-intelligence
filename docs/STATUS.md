...
| P16-S2 | Implemented | `find_proposals` read-only MCP discovery tool: `FindProposalsResult` union type (`{ actionable: Array<{fingerprint,ruleId,subject?}>; count }` | `{ status: "no_analysis"; message }`) + `Session.findProposals(opts?)` in `packages/core/src/mcp/tools.ts`; private `isActionable(ruleId)` mirrors `proposeRefactor` predicate exactly (SHARED_EXTRACTION_RULE_ID || proposalBuilders match); subject projected generically from evidence (`(evidence as {subject?:{name?:string}}).subject?.name` — undefined for shared-extraction, component name for adapter-metric); suppression filter and no_analysis guard reuse existing patterns; sorted fingerprint asc / ruleId asc; `find_proposals` tool registered in `buildMcpServer` (`packages/core/src/mcp/server.ts`) with `ruleId?` + `includeSuppressed?` params; agreement-invariant test locks findProposals ↔ proposeRefactor equivalence; framework-free guard stays green. 79 test files / 780 tests (+13 new). |
