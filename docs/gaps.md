...
- Pipeline: DRY-RUN → TYPECHECK → TESTS → GIT-clean → commit + reversal patch — dry-run ✅ done; verification/apply not implemented
...
| P5 | [docs/superpowers/plans/p5-codemod-apply.md](superpowers/plans/p5-codemod-apply.md) | ✅ Exists; Slices 1–4 complete |
...
12. **Implement P5 Slice 5** — `apply_refactor` verification pipeline.
13. **Resolve §3.4 (Next.js variant guard design)** before P6 adapter planning.
