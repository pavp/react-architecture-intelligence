...
function routeEnrichment(
	routeIds: string[],
	detection: NextDetection = appDetection,
): NextGraphEnrichment {
	const tags = new Map<string, NextTag[]>(
		routeIds.map((id) => [
			id,
			[
				{
					...
					file: `${detection.variant === "pages-router" ? "pages" : "app"}/${id}.tsx`,
				},
			],
		]),
	);
	return {
		nodeTags: tags,
		extraEdges: [],
		roleIndex: new Map([["RouteSegment", [...routeIds].sort()]]),
	};
}
...
test("emits next/route-coupling for App Router route topology breaches", () => {
	...
	const result = createRouteCouplingAnalyzer().analyze(
		input(components, [
			renders("Shell", "Route"),
			renders("Route", "A"),
			renders("Route", "B"),
...
test("supports Pages Router route segments and scopes metrics to route-owned topology", () => {
	...
	const edges = [
		...
		renders("PageRoute", "C"),
		renders("C", "D"),
		renders("Unrelated", "External"),
	];
	const result = createRouteCouplingAnalyzer().analyze(
		input(components, edges, pagesDetection, ["PageRoute"]),
	);
...
test("explains route topology breaches without generic adapter-metric wording", () => {
	const components = [
		component("Route"),
		component("Shell"),
		component("A"),
	...
	const explanation = explainRouteCouplingFinding(
		presented(result.findings[0]!),
	);

	expect(explanation).toMatchObject({
		summary:
			"Route is a route segment with render topology above configured limits: 3 outgoing render links (limit: 2), 3 direct render children (limit: 2), and 4 reachable components (limit: 3).",
		whyItMatters:
		...
		inspectFirst: [
			"Route in app/Route.tsx",
			"route role: app-router in app/Route.tsx",
			"incoming render links observed: 1",
			...
			"observed counts: 1 incoming, 3 outgoing, 3 direct children, 4 reachable components, depth 2",
		],
	});
...
test("stays silent when metrics are below or equal to thresholds", () => {
	const below = createRouteCouplingAnalyzer().analyze(
		input(
			[component("Route"), component("A"), component("B")],
			[renders("Route", "A"), renders("A", "B")],
		),
	);
	const equal = createRouteCouplingAnalyzer().analyze(
		input(
			[
				component("Route"),
				component("Parent"),
...
test("returns diagnostics without findings for mixed-router and non-Next unsupported variants", () => {
	const mixed = createRouteCouplingAnalyzer().analyze(
		input([component("Route")], [], mixedDetection),
	);
	const nonNext = createRouteCouplingAnalyzer().analyze({
	...
	expect(mixed.diagnostics).toEqual([
		{
			...
			supportedVariants: ["app-router", "pages-router"],
			rootDir: ".",
			message:
				"next/route-coupling supports app-router, pages-router, detected mixed-router",
		},
	]);
	expect(nonNext.diagnostics).toEqual([
		{
			kind: "variant-mismatch",
			adapterId: "next",
			analyzerId: ROUTE_COUPLING_RULE_ID,
			...
			rootDir: "",
			message:
				"next/route-coupling supports app-router, pages-router, detected non-next",
		},
	]);
});
...
test("emits metric-only evidence with route roles and render topology references only", () => {
	...
	const evidence = result.findings[0]!.evidence as AdapterMetricEvidence;
	const evidenceWithoutSpan = {
		...evidence,
		subject: { ...evidence.subject, span: undefined },
	};
	...
	expect(evidence).toMatchObject({
		roles: [
			{ role: "RouteSegment", variant: "app-router", file: "app/Route.tsx" },
		],
		topology: {
			directChildIds: ["A", "B", "C"],
			reachableNodeIds: ["A", "B", "C", "D"],
...
test("is pure and exposes findings and diagnostics through return values only", () => {
	const inputValue = Object.freeze(
		input(
			[component("Route"), component("A"), component("B"), component("C")],
			[renders("Route", "A"), renders("Route", "B"), renders("Route", "C")],
		),
	);
	...
	expect(result.findings).toHaveLength(1);
	expect(result.diagnostics).toEqual([]);
	expect(Object.keys(createRouteCouplingAnalyzer()).sort()).toEqual([
		"analyze",
		"ruleId",
	]);
});
...
test("produces deterministic output and traverses render cycles safely", () => {
	...
	const analyzer = createRouteCouplingAnalyzer({
		thresholds: {
			maxFanIn: 0,
			maxFanOut: 1,
			maxDirectChildren: 1,
	...
	const run = (orderedComponents: ComponentNode[], orderedEdges: GraphEdge[]) =>
		analyzer
			.analyze({
				...input(orderedComponents, orderedEdges, appDetection, [
					"RouteA",
					"RouteZ",
				]),
