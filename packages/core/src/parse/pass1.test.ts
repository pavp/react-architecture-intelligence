...
test("extracts custom hook declarations with composed hook calls", () => {
	const r = pass1(
		"hooks.ts",
		`export function useCheckout() { const cart = useCart(); const price = usePrice(); return { cart, price }; }
...
test("records a Span with byte offsets and an astPath", () => {
	const btn = pass1("Button.tsx", SRC).components.find(
		(c) => c.name === "Button",
	)!;
...
test("detects an export-default function component", () => {
	const r = pass1(
		"Card.tsx",
		`export default function Card({ title }) { return <div>{title}</div>; }`,
	);
	expect(r.components.find((x) => x.name === "Card")?.exportKind).toBe(
		"default",
	);
});
...
export default memo(Button);`,
	...
	expect(r.components.find((x) => x.name === "Button")?.exportKind).toBe(
		"default",
	);
});
...
test("default-valued destructured props are captured", () => {
	...
	expect(r.components.find((x) => x.name === "F")?.propNames.sort()).toEqual([
		"a",
		"b",
	]);
});
...
test("collects all nested composition markers", () => {
	const r = pass1(
		"W.tsx",
		`const W = memo(forwardRef(({ x }, ref) => <div>{x}</div>));
export default W;`,
	);
...
`;
		...
		expect(result.patternFacts).toContainEqual(
		...
		expect(result.patternFacts).toContainEqual(
			expect.objectContaining({ kind: "call", callee: "makeThing" }),
		);
		expect(result.patternFacts).toContainEqual(
			expect.objectContaining({ kind: "hook-call", name: "useThing" }),
		);
		expect(result.patternFacts).toContainEqual(
			expect.objectContaining({
		...
		expect(result.patternFacts).toContainEqual(
			expect.objectContaining({
				kind: "jsx",
				tag: "Namespace.Trigger",
				parentTag: "Namespace.Root",
			}),
		);
		expect(result.patternFacts).toContainEqual(
			expect.objectContaining({
		...
		expect(
			result.patternFacts.every(
				(fact) =>
					fact.span.file === "src/Widget.tsx" &&
					fact.span.end > fact.span.start,
			),
		).toBe(true);
	});

	test("extracts call-binding facts for simple call initializers only", () => {
		const result = pass1(
			"src/facts.tsx",
			`
const ThemeContext = createContext(defaultTheme);
const client = api.createClient(config);
const { ignored } = makeThing();
`,
		);
		...
		expect(result.patternFacts).toContainEqual(
			expect.objectContaining({
				kind: "call-binding",
				local: "client",
				callee: "api.createClient",
				declarationKind: "const",
			}),
		);
		expect(result.patternFacts).not.toContainEqual(
			expect.objectContaining({
				kind: "call-binding",
				local: "ignored",
			}),
	...
	test("extracts call-argument facts with deterministic indexes and bounded kinds", () => {
		const result = pass1(
			"src/calls.tsx",
			`
useContext(ThemeContext);
fetch("/api/users", options.headers, makeInit(), { cache: "no-store" });
`,
		);
	...
	test("extracts jsx-attribute facts for literal expression boolean and spread syntax", () => {
		const result = pass1(
			"src/view.tsx",
			`
const props = {};
export function View() {
  return <form method="post" onSubmit={handleSubmit} disabled {...props} />;
}
`,
		);
...
`,
		...
		expect(text).not.toMatch(
			/react\/[\w-]+|ruleId|analyzer|remediation|must refactor|provider\/context|controlled-uncontrolled/i,
		);
	});
});
...
describe("KI-1 fix", () => {
	test("SC-1: route handler GET is NOT admitted as a component", () => {
		const source = readFileSync(
			join(FIX, "duplication/route-handlers/GET.ts"),
			"utf8",
		);
	...
	test("SC-2: forwardRef component IS detected (regression guard)", () => {
		const source = readFileSync(
			join(FIX, "truepositives/forwardref-components/Button.tsx"),
			"utf8",
		);
	...
	test("SC-4: plain JSX-returning function component IS detected", () => {
		const result = pass1(
			"Header.tsx",
			`export function Header({ title }: { title: string }) { return <header><h1>{title}</h1></header>; }`,
		);
		expect(result.components.length).toBe(1);
